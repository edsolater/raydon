...
export async function switchRpc(customizedEndPoint: Endpoint) {
  try {
    ...
    const { currentEndPoint } = connectionAtom.get()
    ...
    if (currentEndPoint === customizedEndPoint) {
      ...
      const isUserAdded = connectionAtom
        .get()
...
export async function deleteRpc(endPointUrl: string) {
  ...
  connectionAtom.set({
    availableEndPoints: (connectionAtom.get().availableEndPoints ?? []).filter((i) => i.url !== endPointUrl)
  })
  return true
}
